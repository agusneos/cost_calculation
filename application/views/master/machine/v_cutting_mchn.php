...
<!-- Data Grid -->
<table id="grid-master_cutting_mchn"
    data-options="pageSize:30, rownumbers:true, singleSelect:true, fit:true, fitColumns:false, toolbar:toolbar_master_cutting_mchn">
    <thead>
        <tr>
            <th data-options="field:'ck',checkbox:true" ></th>
            <th data-options="field:'Kode_mchncutt'"           width="100" align="center" sortable="true">Kode Mesin</th>
            ...
            <th data-options="field:'Cutting_depr_cost'"       width="100" align="center" sortable="true">Depr. Cost</th>
        </tr>
    </thead>
</table>

<script type="text/javascript">
    var toolbar_master_cutting_mchn = [{
        text:'New',
        iconCls:'icon-new_file',
        handler:function(){mastercutting_mchnCreate();}
    },{
        text:'Edit',
        iconCls:'icon-edit',
        handler:function(){mastercutting_mchnUpdate();}
    },{
        text:'Delete',
        iconCls:'icon-cancel',
        handler:function(){mastercutting_mchnHapus();}
    },{
        text:'Refresh',
        iconCls:'icon-reload',
        handler:function(){$('#grid-master_cutting_mchn').datagrid('reload');}
    }];
    
    $('#grid-master_cutting_mchn').datagrid({view:scrollview,remoteFilter:true,
        url:'<?php echo site_url('master/machine/cutting_mchn/index'); ?>?grid=true'})
    ...
    function mastercutting_mchnCreate() {
        $('#dlg-master_cutting_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Tambah Data');
        $('#fm-master_cutting_mchn').form('clear');
        url = '<?php echo site_url('master/machine/cutting_mchn/create'); ?>';
        ...
        $('#Length_range').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
                $('#Mchn_cutting').next().find('input').focus();
            }
        });
        $('#Mchn_cutting').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
        ...
        $('#Productivity_ratio').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){
                ...
                $('#Cutting_depr_cost').numberbox('setValue', dprc);
            }
        });
    }
    
    
    function mastercutting_mchnUpdate() {
        var row = $('#grid-master_cutting_mchn').datagrid('getSelected');
        if(row){
            $('#dlg-master_cutting_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Edit Data');
            $('#fm-master_cutting_mchn').form('load',row);
            url = '<?php echo site_url('master/machine/cutting_mchn/update'); ?>/' + row.Kode_mchncutt; 
    ...
    function mastercutting_mchnSave(){
        $('#fm-master_cutting_mchn').form('submit',{
            url: url,
            onSubmit: function(){
            ...
            success: function(result){
                ...
                if(result.success){
                    $('#dlg-master_cutting_mchn').dialog('close');
                    $('#grid-master_cutting_mchn').datagrid('reload');
                    $.messager.show({
                        title: 'Info',
    ...
    function mastercutting_mchnHapus(){
        var row = $('#grid-master_cutting_mchn').datagrid('getSelected');
        if (row){
            $.messager.confirm('Konfirmasi','Anda yakin ingin menghapus machine '+row.Kode_mchncutt+' ?',function(r){
                if (r){
                    $.post('<?php echo site_url('master/machine/cutting_mchn/delete'); ?>',{Kode_mchncutt:row.Kode_mchncutt},function(result){
                        if (result.success){
                            $('#grid-master_cutting_mchn').datagrid('reload');
                            $.messager.show({
                                title: 'Info',
...
<div id="dlg-master_cutting_mchn" class="easyui-dialog" style="width:400px; height:500px; padding: 10px 20px" closed="true" buttons="#dlg-buttons-master_cutting_mchn">
    <form id="fm-master_cutting_mchn" method="post" novalidate>   
        <div class="fitem">
        ...
        <div class="fitem">
            <label for="type">Mesin Cutting</label>
            <input type="text" id="Mchn_cutting" name="Mchn_cutting" class="easyui-textbox" required="true"/>
        </div>
        <div class="fitem">
...
<!-- Dialog Button -->
<div id="dlg-buttons-master_cutting_mchn">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" id="oksave" iconCls="icon-ok" onclick="mastercutting_mchnSave()">Simpan</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-cancel" onclick="javascript:$('#dlg-master_cutting_mchn').dialog('close')">Batal</a>
</div>

<!-- End of file v_cutting_mchn.php -->
<!-- Location: ./application/views/master/machine/v_cutting_mchn.php -->