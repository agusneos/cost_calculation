...
<!-- Data Grid -->
<table id="grid-master_heading_mchn"
    data-options="pageSize:30, rownumbers:true, singleSelect:true, fit:true, fitColumns:false, toolbar:toolbar_master_heading_mchn">
    <thead>
        <tr>
            <th data-options="field:'ck',checkbox:true" ></th>
            <th data-options="field:'Kode_mchnhead'"           width="100" align="center" sortable="true">Kode Mesin</th>
            <th data-options="field:'Gol_mchn_head'"           width="100" align="center" sortable="true">Golongan Mesin</th>
            ...
            <th data-options="field:'Mchn_heading'"            width="100" align="center" sortable="true">Mesin Heading</th>
            ...
            <th data-options="field:'Heading_depr_cost'"       width="100" align="center" sortable="true">Depr. Cost</th>
        </tr>
    </thead>
</table>

<script type="text/javascript">
    var toolbar_master_heading_mchn = [{
        text:'New',
        iconCls:'icon-new_file',
        handler:function(){masterheading_mchnCreate();}
    },{
        text:'Edit',
        iconCls:'icon-edit',
        handler:function(){masterheading_mchnUpdate();}
    },{
        text:'Delete',
        iconCls:'icon-cancel',
        handler:function(){masterheading_mchnHapus();}
    },{
        text:'Refresh',
        iconCls:'icon-reload',
        handler:function(){$('#grid-master_heading_mchn').datagrid('reload');}
    }];
    
    $('#grid-master_heading_mchn').datagrid({view:scrollview,remoteFilter:true,
        url:'<?php echo site_url('master/machine/heading_mchn/index'); ?>?grid=true'})
    ...
    function masterheading_mchnCreate() {
        $('#dlg-master_heading_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Tambah Data');
        $('#fm-master_heading_mchn').form('clear');
        url = '<?php echo site_url('master/machine/heading_mchn/create'); ?>';
        ...
        $('#Length_range').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
                $('#Mchn_heading').next().find('input').focus();
            }
        });
        $('#Mchn_heading').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
        ...
        $('#Productivity_ratio').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){
                ...
                $('#Heading_depr_cost').numberbox('setValue', dprc);
            }
        });
    }
    
    
    function masterheading_mchnUpdate() {
        var row = $('#grid-master_heading_mchn').datagrid('getSelected');
        if(row){
            $('#dlg-master_heading_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Edit Data');
            $('#fm-master_heading_mchn').form('load',row);
            url = '<?php echo site_url('master/machine/heading_mchn/update'); ?>/' + row.Kode_mchnhead; 
    ...
    function masterheading_mchnSave(){
        $('#fm-master_heading_mchn').form('submit',{
            url: url,
            onSubmit: function(){
            ...
            success: function(result){
                ...
                if(result.success){
                    $('#dlg-master_heading_mchn').dialog('close');
                    $('#grid-master_heading_mchn').datagrid('reload');
                    $.messager.show({
                        title: 'Info',
    ...
    function masterheading_mchnHapus(){
        var row = $('#grid-master_heading_mchn').datagrid('getSelected');
        if (row){
            $.messager.confirm('Konfirmasi','Anda yakin ingin menghapus machine '+row.Kode_mchnhead+' ?',function(r){
                if (r){
                    $.post('<?php echo site_url('master/machine/heading_mchn/delete'); ?>',{Kode_mchnhead:row.Kode_mchnhead},function(result){
                        if (result.success){
                            $('#grid-master_heading_mchn').datagrid('reload');
                            $.messager.show({
                                title: 'Info',
...
<div id="dlg-master_heading_mchn" class="easyui-dialog" style="width:400px; height:500px; padding: 10px 20px" closed="true" buttons="#dlg-buttons-master_heading_mchn">
    <form id="fm-master_heading_mchn" method="post" novalidate>   
        <div class="fitem">
            <label for="type">Golongan Mesin</label>
            <input id="Gol_mchn_head" name="Gol_mchn_head" class="easyui-combobox" data-options=" 
            url:'<?php echo site_url('master/machine/heading_mchn/enumGolmesin'); ?>',
        ...
        <div class="fitem">
            <label for="type">Mesin Heading</label>
            <input type="text" id="Mchn_heading" name="Mchn_heading" class="easyui-textbox" required="true"/>
        </div>
        <div class="fitem">
...
<!-- Dialog Button -->
<div id="dlg-buttons-master_heading_mchn">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" id="oksave" iconCls="icon-ok" onclick="masterheading_mchnSave()">Simpan</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-cancel" onclick="javascript:$('#dlg-master_heading_mchn').dialog('close')">Batal</a>
</div>

<!-- End of file v_heading_mchn.php -->
<!-- Location: ./application/views/master/machine/v_heading_mchn.php -->