...
<!-- Data Grid -->
<table id="grid-master_rolling_mchn"
    data-options="pageSize:30, rownumbers:true, singleSelect:true, fit:true, fitColumns:false, toolbar:toolbar_master_rolling_mchn">
    <thead>
        <tr>
            <th data-options="field:'ck',checkbox:true" ></th>
            <th data-options="field:'Kode_mchnroll'"               width="100" align="center" sortable="true">Kode Mesin</th>
            <th data-options="field:'Gol_mchn_roll'"           width="100" align="center" sortable="true">Golongan Mesin</th>
            ...
            <th data-options="field:'Mchn_rolling'"            width="100" align="center" sortable="true">Mesin rolling</th>
            ...
            <th data-options="field:'Rolling_depr_cost'"       width="100" align="center" sortable="true">Depr. Cost</th>
        </tr>
    </thead>
</table>

<script type="text/javascript">
    var toolbar_master_rolling_mchn = [{
        text:'New',
        iconCls:'icon-new_file',
        handler:function(){masterrolling_mchnCreate();}
    },{
        text:'Edit',
        iconCls:'icon-edit',
        handler:function(){masterrolling_mchnUpdate();}
    },{
        text:'Delete',
        iconCls:'icon-cancel',
        handler:function(){masterrolling_mchnHapus();}
    },{
        text:'Refresh',
        iconCls:'icon-reload',
        handler:function(){$('#grid-master_rolling_mchn').datagrid('reload');}
    }];
    
    $('#grid-master_rolling_mchn').datagrid({view:scrollview,remoteFilter:true,
        url:'<?php echo site_url('master/machine/rolling_mchn/index'); ?>?grid=true'})
    ...
    function masterrolling_mchnCreate() {
        $('#dlg-master_rolling_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Tambah Data');
        $('#fm-master_rolling_mchn').form('clear');
        url = '<?php echo site_url('master/machine/rolling_mchn/create'); ?>';
        ...
        $('#Length_range').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
                $('#Mchn_rolling').next().find('input').focus();
            }
        });
        $('#Mchn_rolling').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
        ...
        $('#Productivity_ratio').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){
                ...
                $('#Rolling_depr_cost').numberbox('setValue', dprc);
            }
        });
    }
    
    
    function masterrolling_mchnUpdate() {
        var row = $('#grid-master_rolling_mchn').datagrid('getSelected');
        if(row){
            $('#dlg-master_rolling_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Edit Data');
            $('#fm-master_rolling_mchn').form('load',row);
            url = '<?php echo site_url('master/machine/rolling_mchn/update'); ?>/' + row.Kode_mchnroll; 
    ...
    function masterrolling_mchnSave(){
        $('#fm-master_rolling_mchn').form('submit',{
            url: url,
            onSubmit: function(){
            ...
            success: function(result){
                ...
                if(result.success){
                    $('#dlg-master_rolling_mchn').dialog('close');
                    $('#grid-master_rolling_mchn').datagrid('reload');
                    $.messager.show({
                        title: 'Info',
    ...
    function masterrolling_mchnHapus(){
        var row = $('#grid-master_rolling_mchn').datagrid('getSelected');
        if (row){
            $.messager.confirm('Konfirmasi','Anda yakin ingin menghapus machine '+row.Kode_mchnroll+' ?',function(r){
                if (r){
                    $.post('<?php echo site_url('master/machine/rolling_mchn/delete'); ?>',{Kode_mchnroll:row.Kode_mchnroll},function(result){
                        if (result.success){
                            $('#grid-master_rolling_mchn').datagrid('reload');
                            $.messager.show({
                                title: 'Info',
...
<div id="dlg-master_rolling_mchn" class="easyui-dialog" style="width:400px; height:500px; padding: 10px 20px" closed="true" buttons="#dlg-buttons-master_rolling_mchn">
    <form id="fm-master_rolling_mchn" method="post" novalidate>   
        <div class="fitem">
            <label for="type">Golongan Mesin</label>
            <input id="Gol_mchn_roll" name="Gol_mchn_roll" class="easyui-combobox" data-options=" 
            url:'<?php echo site_url('master/machine/rolling_mchn/enumGolmesin'); ?>',
        ...
        <div class="fitem">
            <label for="type">Mesin Rolling</label>
            <input type="text" id="Mchn_rolling" name="Mchn_rolling" class="easyui-textbox" required="true"/>
        </div>
         <div class="fitem">
...
<!-- Dialog Button -->
<div id="dlg-buttons-master_rolling_mchn">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" id="oksave" iconCls="icon-ok" onclick="masterrolling_mchnSave()">Simpan</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-cancel" onclick="javascript:$('#dlg-master_rolling_mchn').dialog('close')">Batal</a>
</div>

<!-- End of file v_rolling_mchn.php -->
<!-- Location: ./application/views/master/machine/v_rolling_mchn.php -->