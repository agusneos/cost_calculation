...
<!-- Data Grid -->
<table id="grid-master_slotting_mchn"
    data-options="pageSize:30, rownumbers:true, singleSelect:true, fit:true, fitColumns:false, toolbar:toolbar_master_slotting_mchn">
    <thead>
        <tr>
            <th data-options="field:'ck',checkbox:true" ></th>
            <th data-options="field:'Kode_mchnslott'"           width="100" align="center" sortable="true">Kode Mesin</th>
            ...
            <th data-options="field:'Slotting_depr_cost'"       width="100" align="center" sortable="true">Depr. Cost</th>
        </tr>
    </thead>
</table>

<script type="text/javascript">
    var toolbar_master_slotting_mchn = [{
        text:'New',
        iconCls:'icon-new_file',
        handler:function(){masterslotting_mchnCreate();}
    },{
        text:'Edit',
        iconCls:'icon-edit',
        handler:function(){masterslotting_mchnUpdate();}
    },{
        text:'Delete',
        iconCls:'icon-cancel',
        handler:function(){masterslotting_mchnHapus();}
    },{
        text:'Refresh',
        iconCls:'icon-reload',
        handler:function(){$('#grid-master_slotting_mchn').datagrid('reload');}
    }];
    
    $('#grid-master_slotting_mchn').datagrid({view:scrollview,remoteFilter:true,
        url:'<?php echo site_url('master/machine/slotting_mchn/index'); ?>?grid=true'})
    ...
    function masterslotting_mchnCreate() {
        $('#dlg-master_slotting_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Tambah Data');
        $('#fm-master_slotting_mchn').form('clear');
        url = '<?php echo site_url('master/machine/slotting_mchn/create'); ?>';
        ...
        $('#Length_range').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
                $('#Mchn_slotting').next().find('input').focus();
            }
        });
        $('#Mchn_slotting').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
        ...
        $('#Productivity_ratio').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){
                ...
                $('#Slotting_depr_cost').numberbox('setValue', dprc);
            }
        });
    }
    
    
    function masterslotting_mchnUpdate() {
        var row = $('#grid-master_slotting_mchn').datagrid('getSelected');
        if(row){
            $('#dlg-master_slotting_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Edit Data');
            $('#fm-master_slotting_mchn').form('load',row);
            url = '<?php echo site_url('master/machine/slotting_mchn/update'); ?>/' + row.Kode_mchnslott; 
    ...
    function masterslotting_mchnSave(){
        $('#fm-master_slotting_mchn').form('submit',{
            url: url,
            onSubmit: function(){
            ...
            success: function(result){
                ...
                if(result.success){
                    $('#dlg-master_slotting_mchn').dialog('close');
                    $('#grid-master_slotting_mchn').datagrid('reload');
                    $.messager.show({
                        title: 'Info',
    ...
    function masterslotting_mchnHapus(){
        var row = $('#grid-master_slotting_mchn').datagrid('getSelected');
        if (row){
            $.messager.confirm('Konfirmasi','Anda yakin ingin menghapus machine '+row.Kode_mchnslott+' ?',function(r){
                if (r){
                    $.post('<?php echo site_url('master/machine/slotting_mchn/delete'); ?>',{Kode_mchnslott:row.Kode_mchnslott},function(result){
                        if (result.success){
                            $('#grid-master_slotting_mchn').datagrid('reload');
                            $.messager.show({
                                title: 'Info',
...
<div id="dlg-master_slotting_mchn" class="easyui-dialog" style="width:400px; height:500px; padding: 10px 20px" closed="true" buttons="#dlg-buttons-master_slotting_mchn">
    <form id="fm-master_slotting_mchn" method="post" novalidate>   
        <div class="fitem">
        ...
        <div class="fitem">
            <label for="type">Mesin Slotting</label>
            <input type="text" id="Mchn_slotting" name="Mchn_slotting" class="easyui-textbox" required="true"/>
        </div>
        <div class="fitem">
...
<!-- Dialog Button -->
<div id="dlg-buttons-master_slotting_mchn">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" id="oksave" iconCls="icon-ok" onclick="masterslotting_mchnSave()">Simpan</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-cancel" onclick="javascript:$('#dlg-master_slotting_mchn').dialog('close')">Batal</a>
</div>

<!-- End of file v_slotting_mchn.php -->
<!-- Location: ./application/views/master/machine/v_slotting_mchn.php -->