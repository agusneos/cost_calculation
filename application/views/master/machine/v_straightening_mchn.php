...
<!-- Data Grid -->
<table id="grid-master_straightening_mchn"
    data-options="pageSize:30, rownumbers:true, singleSelect:true, fit:true, fitColumns:false, toolbar:toolbar_master_straightening_mchn">
    <thead>
        <tr>
            <th data-options="field:'ck',checkbox:true" ></th>
            <th data-options="field:'Kode_mchnstraighten'"           width="100" align="center" sortable="true">Kode Mesin</th>
            ...
            <th data-options="field:'Straightening_depr_cost'"       width="100" align="center" sortable="true">Depr. Cost</th>
        </tr>
    </thead>
</table>

<script type="text/javascript">
    var toolbar_master_straightening_mchn = [{
        text:'New',
        iconCls:'icon-new_file',
        handler:function(){masterstraightening_mchnCreate();}
    },{
        text:'Edit',
        iconCls:'icon-edit',
        handler:function(){masterstraightening_mchnUpdate();}
    },{
        text:'Delete',
        iconCls:'icon-cancel',
        handler:function(){masterstraightening_mchnHapus();}
    },{
        text:'Refresh',
        iconCls:'icon-reload',
        handler:function(){$('#grid-master_straightening_mchn').datagrid('reload');}
    }];
    
    $('#grid-master_straightening_mchn').datagrid({view:scrollview,remoteFilter:true,
        url:'<?php echo site_url('master/machine/straightening_mchn/index'); ?>?grid=true'})
    ...
    function masterstraightening_mchnCreate() {
        $('#dlg-master_straightening_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Tambah Data');
        $('#fm-master_straightening_mchn').form('clear');
        url = '<?php echo site_url('master/machine/straightening_mchn/create'); ?>';
        ...
        $('#Length_range').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
                $('#Mchn_straightening').next().find('input').focus();
            }
        });
          $('#Mchn_straightening').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
        ...
        $('#Productivity_ratio').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){
                ...
                $('#Straightening_depr_cost').numberbox('setValue', dprc);
            }
        });
    }
    
    
    function masterstraightening_mchnUpdate() {
        var row = $('#grid-master_straightening_mchn').datagrid('getSelected');
        if(row){
            $('#dlg-master_straightening_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Edit Data');
            $('#fm-master_straightening_mchn').form('load',row);
            url = '<?php echo site_url('master/machine/straightening_mchn/update'); ?>/' + row.Kode_mchnstraighten; 
          ...
          $('#Mchn_price').numberbox('textbox').keypress(function(e){
           ...
           $('#Output_per_min').numberbox('textbox').keyup(function(e){
               if (e.keyCode == 13){                
                   var prc = $('#Mchn_price').numberbox('getValue');
                   var dpm = eval(prc)/(8*12);
                   $('#Depr_per_month').numberbox('setValue', dpm);
           ...
           $('#Productivity_ratio').numberbox('textbox').keyup(function(e){
               if (e.keyCode == 13){                
                   var opmin = $('#Output_per_min').numberbox('getValue');
                   var wt = $('#Working_time').numberbox('getValue');
                   var opd = eval(opmin)*60*14;
    ...
    function masterstraightening_mchnSave(){
        $('#fm-master_straightening_mchn').form('submit',{
            url: url,
            onSubmit: function(){
            ...
            success: function(result){
                ...
                if(result.success){
                    $('#dlg-master_straightening_mchn').dialog('close');
                    $('#grid-master_straightening_mchn').datagrid('reload');
                    $.messager.show({
                        title: 'Info',
    ...
    function masterstraightening_mchnHapus(){
        var row = $('#grid-master_straightening_mchn').datagrid('getSelected');
        if (row){
            $.messager.confirm('Konfirmasi','Anda yakin ingin menghapus machine '+row.Kode_mchnstraighten+' ?',function(r){
                if (r){
                    $.post('<?php echo site_url('master/machine/straightening_mchn/delete'); ?>',{Kode_mchnstraighten:row.Kode_mchnstraighten},function(result){
                        if (result.success){
                            $('#grid-master_straightening_mchn').datagrid('reload');
                            $.messager.show({
                                title: 'Info',
...
<div id="dlg-master_straightening_mchn" class="easyui-dialog" style="width:400px; height:500px; padding: 10px 20px" closed="true" buttons="#dlg-buttons-master_straightening_mchn">
    <form id="fm-master_straightening_mchn" method="post" novalidate>   
        <div class="fitem">
        ...
        <div class="fitem">
            <label for="type">Mesin Straightening</label>
            <input type="text" id="Mchn_straightening" name="Mchn_straightening" class="easyui-textbox" required="true"/>
        </div>
        <div class="fitem">
...
<!-- Dialog Button -->
<div id="dlg-buttons-master_straightening_mchn">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" id="oksave" iconCls="icon-ok" onclick="masterstraightening_mchnSave()">Simpan</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-cancel" onclick="javascript:$('#dlg-master_straightening_mchn').dialog('close')">Batal</a>
</div>

<!-- End of file v_straightening_mchn.php -->
<!-- Location: ./application/views/master/machine/v_straightening_mchn.php -->