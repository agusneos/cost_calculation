...
<!-- Data Grid -->
<table id="grid-master_trimming_mchn"
    data-options="pageSize:30, rownumbers:true, singleSelect:true, fit:true, fitColumns:false, toolbar:toolbar_master_trimming_mchn">
    <thead>
        <tr>
            <th data-options="field:'ck',checkbox:true" ></th>
            <th data-options="field:'Kode_mchntrimm'"          width="100" align="center" sortable="true">Kode Mesin</th>
            ...
            <th data-options="field:'Dandori_time'"            width="100" align="center" sortable="true">Dandori time</th><th data-options="field:'Depr_per_month'"          width="100" align="center" formatter="price" sortable="true">Depr./bulan</th>
            ...
            <th data-options="field:'Trimming_depr_cost'"       width="100" align="center" sortable="true">Depr. Cost</th>
        </tr>
    </thead>
</table>

<script type="text/javascript">
    var toolbar_master_trimming_mchn = [{
        text:'New',
        iconCls:'icon-new_file',
        handler:function(){mastertrimming_mchnCreate();}
    },{
        text:'Edit',
        iconCls:'icon-edit',
        handler:function(){mastertrimming_mchnUpdate();}
    },{
        text:'Delete',
        iconCls:'icon-cancel',
        handler:function(){mastertrimming_mchnHapus();}
    },{
        text:'Refresh',
        iconCls:'icon-reload',
        handler:function(){$('#grid-master_trimming_mchn').datagrid('reload');}
    }];
    
    $('#grid-master_trimming_mchn').datagrid({view:scrollview,remoteFilter:true,
        url:'<?php echo site_url('master/machine/trimming_mchn/index'); ?>?grid=true'})
    ...
    function mastertrimming_mchnCreate() {
        $('#dlg-master_trimming_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Tambah Data');
        $('#fm-master_trimming_mchn').form('clear');
        url = '<?php echo site_url('master/machine/trimming_mchn/create'); ?>';
        ...
        $('#Length_range').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
                $('#Mchn_trimming').next().find('input').focus();
            }
        });
        $('#Mchn_trimming').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){                
        ...
        $('#Productivity_ratio').numberbox('textbox').keypress(function(e){
            if (e.keyCode == 13){
                ...
                $('#Trimming_depr_cost').numberbox('setValue', dprc);
            }
        });
    }
    
    
    function mastertrimming_mchnUpdate() {
        var row = $('#grid-master_trimming_mchn').datagrid('getSelected');
        if(row){
            $('#dlg-master_trimming_mchn').dialog({modal: true}).dialog('open').dialog('setTitle','Edit Data');
            $('#fm-master_trimming_mchn').form('load',row);
            url = '<?php echo site_url('master/machine/trimming_mchn/update'); ?>/' + row.Kode_mchntrimm; 
    ...
    function mastertrimming_mchnSave(){
        $('#fm-master_trimming_mchn').form('submit',{
            url: url,
            onSubmit: function(){
            ...
            success: function(result){
                ...
                if(result.success){
                    $('#dlg-master_trimming_mchn').dialog('close');
                    $('#grid-master_trimming_mchn').datagrid('reload');
                    $.messager.show({
                        title: 'Info',
    ...
    function mastertrimming_mchnHapus(){
        var row = $('#grid-master_trimming_mchn').datagrid('getSelected');
        if (row){
            $.messager.confirm('Konfirmasi','Anda yakin ingin menghapus machine '+row.Kode_mchntrimm+' ?',function(r){
                if (r){
                    $.post('<?php echo site_url('master/machine/trimming_mchn/delete'); ?>',{Kode_mchntrimm:row.Kode_mchntrimm},function(result){
                        if (result.success){
                            $('#grid-master_trimming_mchn').datagrid('reload');
                            $.messager.show({
                                title: 'Info',
...
<div id="dlg-master_trimming_mchn" class="easyui-dialog" style="width:400px; height:500px; padding: 10px 20px" closed="true" buttons="#dlg-buttons-master_trimming_mchn">
    <form id="fm-master_trimming_mchn" method="post" novalidate>   
        <div class="fitem">
        ...
        <div class="fitem">
            <label for="type">Mesin Trimming</label>
            <input type="text" id="Mchn_trimming" name="Mchn_trimming" class="easyui-textbox" required="true"/>
        </div>
        <div class="fitem">
...
<!-- Dialog Button -->
<div id="dlg-buttons-master_trimming_mchn">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" id="oksave" iconCls="icon-ok" onclick="mastertrimming_mchnSave()">Simpan</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-cancel" onclick="javascript:$('#dlg-master_trimming_mchn').dialog('close')">Batal</a>
</div>

<!-- End of file v_trimming_mchn.php -->
<!-- Location: ./application/views/master/machine/v_trimming_mchn.php -->