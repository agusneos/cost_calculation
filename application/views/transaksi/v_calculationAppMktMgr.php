...
<table id="grid-transaksi_calculationAppMktMgr"
       data-options="pageSize:50, rownumbers:true, singleSelect:true, fit:true, fitColumns:false, toolbar:toolbar_transaksi_calculationAppMktMgr, 
...
<script type="text/javascript">
    var toolbar_transaksi_calculationAppMktMgr = [{
            text: 'Approve',
            iconCls: 'icon-new_file',
            handler: function() {
                transaksicalculationAppMktMgrApprove();
            }
            }, {
            text: 'Refresh',
            iconCls: 'icon-reload',
            handler: function() {
                $('#grid-transaksi_calculationAppMktMgr').datagrid('reload');
            }
    }]        
    $('#grid-transaksi_calculationAppMktMgr').datagrid({view: scrollview, remoteFilter: true,
        url: '<?php echo site_url('transaksi/calculationAppMktMgr/index'); ?>?grid=true'})
    ...
    function transaksicalculationAppMktMgrApprove() {
        var row = $('#grid-transaksi_calculationAppMktMgr').datagrid('getSelected');
        if (row) {
        $('#dlg-transaksi_calculationAppMktMgr').dialog({modal: true}).dialog('open').dialog('setTitle', 'Approve Calculation');
        $('#fm-transaksi_calculationAppMktMgr').form('load', row);
            url = '<?php echo site_url('transaksi/calculationAppMktMgr/approve'); ?>/' + row.Id;
        }              
        else
        {
    ...
    function transaksicalculationAppMktMgrSave() {
        $('#fm-transaksi_calculationAppMktMgr').form('submit', {
            url: url,
            onSubmit: function() {
            ...
            success: function(result) {
                ...
                if (result.success) {
                    $('#dlg-transaksi_calculationAppMktMgr').dialog('close');
                    $('#grid-transaksi_calculationAppMktMgr').datagrid('reload');
                    $.messager.show({
                        title: 'Info',
...
<!-- ----------- -->
<div id="dlg-transaksi_calculationAppMktMgr" class="easyui-dialog" style="width:350px; height:250px;" closed="true" buttons="#dlg-buttons-transaksi_calculationAppMktMgr">
    <form id="fm-transaksi_calculationAppMktMgr" method="post" novalidate>        
     <div class="fitem">
             <label for="type">Approval?</label>
             <input id="Approval_Mkt_Mgr" name="Approval_Mkt_Mgr" class="easyui-combobox" data-options=" 
                                   url:'<?php echo site_url('transaksi/calculationAppMktMgr/enumCalculationAppMktMgr'); ?>',
...
<!-- Dialog Button -->
<div id="dlg-buttons-transaksi_calculationAppMktMgr">
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-ok" onclick="transaksicalculationAppMktMgrSave()">Simpan</a>
    <a href="javascript:void(0)" class="easyui-linkbutton" data-options="width:75" iconCls="icon-cancel" onclick="javascript:$('#dlg-transaksi_calculationAppMktMgr').dialog('close')">Batal</a>
</div>

<!-- End of file v_calculationAppMktMgr.php -->
<!-- Location: ./application/views/transaksi/v_calculationAppMktMgr.php -->       
